...
'''
    ...
    async def get_combined_prediction(self, symbol: str, signal_data: Dict[str, Any]) -> Dict[str, float]:
        """
        Get combined win rate prediction from all agents
        
        Args:
            symbol: Trading symbol
            signal_data: Aggregated signal data
            
        Returns:
            Dict with long_win_rate, short_win_rate, and confidence
        """
        try:
            # Extract agent-specific data from signal
            kingfisher_data = signal_data.get('kingfisher_data', {})
            cryptometer_data = signal_data.get('cryptometer_data', {})
            riskmetric_data = signal_data.get('riskmetric_data', {})
            
            # Get predictions from each agent (in parallel)
            predictions = await asyncio.gather(
                self.predict_kingfisher_win_rate(symbol, kingfisher_data),
                self.predict_cryptometer_win_rate(symbol, cryptometer_data), 
                self.predict_riskmetric_win_rate(symbol, riskmetric_data),
                return_exceptions=True
            )
            
            # Filter out any exceptions and get valid predictions
            valid_predictions = []
            for pred in predictions:
                if isinstance(pred, AIWinRatePrediction):
                    valid_predictions.append(pred)
            
            if not valid_predictions:
                # All predictions failed, return fallback
                logger.warning(f"All AI predictions failed for {symbol}, using fallback")
                return {'long_win_rate': 50.0, 'short_win_rate': 50.0, 'confidence': 0.3}
            ...
            weighted_long_rate = 0
            weighted_short_rate = 0
            total_confidence = 0
            total_weight = 0
            
            for pred in valid_predictions:
                weight = weights.get(pred.agent_type, 0.33)
                
                # Convert prediction to long/short rates based on direction
                if pred.direction == 'long':
                    long_rate = pred.win_rate_prediction
                    short_rate = 100 - pred.win_rate_prediction
                elif pred.direction == 'short':
                    long_rate = 100 - pred.win_rate_prediction  
                    short_rate = pred.win_rate_prediction
                else:  # neutral
                    long_rate = pred.win_rate_prediction
                    short_rate = pred.win_rate_prediction
                
                weighted_long_rate += long_rate * weight
                weighted_short_rate += short_rate * weight
                total_confidence += pred.confidence * weight
                total_weight += weight
            
            # Normalize by total weight
            if total_weight > 0:
                final_long_rate = weighted_long_rate / total_weight
                final_short_rate = weighted_short_rate / total_weight
                final_confidence = total_confidence / total_weight
            else:
                final_long_rate = 50.0
                final_short_rate = 50.0
                final_confidence = 0.3
            
            logger.info(f"💡 Combined AI prediction for {symbol}: Long={final_long_rate:.1f}%, Short={final_short_rate:.1f}%")
            
            return {
                'long_win_rate': final_long_rate,
                'short_win_rate': final_short_rate,
                'confidence': final_confidence
            }
            
        except Exception as e:
            logger.error(f"Error in combined AI prediction for {symbol}: {e}")
            return {'long_win_rate': 50.0, 'short_win_rate': 50.0, 'confidence': 0.3}
