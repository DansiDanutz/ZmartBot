...
class AirtableService:
    ...
    async def store_image_analysis(self, analysis_data: Dict[str, Any]) -> bool:
        ...
        try:
            # Prepare record data using actual KingFisher table fields
            record = {
                "fields": {
                    "Symbol": analysis_data.get("symbol", ""),
                    "Liquidation_Map": json.dumps(analysis_data.get("liquidation_clusters", [])),
                    "LiqRatios_long_term": analysis_data.get("long_term_ratios", ""),
                    "LiqRatios_short_term": analysis_data.get("short_term_ratios", ""),
                    "RSI_Heatmap": analysis_data.get("rsi_heatmap", "")
                    # Removed "Lie_Heatmap" due to unknown field error
                    # "Summary": analysis_data.get("summary", "")  # Removed due to AI field validation issues
                }
            }
