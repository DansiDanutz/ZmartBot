...
const SimpleChart = ({ 
  ...
  const [zoomLevel, setZoomLevel] = useState(0.9) // 0.9 = 90% (zoomed out 10% by default)
  ...
  useEffect(() => {
    try {
      ...
      const pricesForCrossover = data.map(candle => parseFloat(candle.close || 0))
      checkEmaCrossovers(pricesForCrossover)

      // Process data for Chart.js
      console.log('🔍 Processing chart data:', data.slice(0, 3))
      const labels = data.map((candle, index) => {
        try {
          const timestamp = candle.time ? candle.time : (candle.timestamp || Date.now())
                      // For 24h chart, always show dates
          ...
          if (index < 3) {
            console.log(`🔍 Label ${index}: candle.time=${candle.time}, timestamp=${timestamp}, timeString=${timeString}`)
          }
          return timeString
        } catch (error) {
      ...
      const prices = data.map(candle => parseFloat(candle.close || 0))
      const volumes = data.map(candle => parseFloat(candle.volume || 0))
  ...
  }, [data, symbol, activeIndicators, chartType, zoomLevel])
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (context) => {
            // Handle different Chart.js context formats
            let tooltipContext = context
            if (Array.isArray(context)) {
              tooltipContext = context[0]
            } else if (context && context.length > 0) {
              tooltipContext = context[0]
            }
            
            if (!tooltipContext || !tooltipContext.label) {
              return `${symbol} - No Data`
            }
            return `${symbol} - ${tooltipContext.label}`
          },
          label: (context) => {
            ...
            console.log('🔍 Processed tooltip context:', tooltipContext)
            
            // Add proper null checks
            if (!tooltipContext || typeof tooltipContext.dataIndex === 'undefined') {
              console.log('🔍 Tooltip: No valid context or dataIndex')
              return ['No data available']
            }
            
            const index = tooltipContext.dataIndex
            console.log(`🔍 Tooltip index: ${index}, data length: ${data.length}`)
            if (index < 0 || index >= data.length) {
              console.log('🔍 Tooltip: Index out of range')
              return ['Data index out of range']
            }
            
            const candle = data[index]
            console.log('🔍 Tooltip candle:', candle)
            if (!candle) {
              console.log('🔍 Tooltip: No candle data')
              return ['Candle data not available']
            }
            
            // Check if this is the main price dataset or an indicator
            const dataset = tooltipContext.dataset
            const value = tooltipContext.parsed?.y
            
            // Only show base price data for the main price dataset
            if (dataset && dataset.label && dataset.label.includes('BTCUSDT Price')) {
              const tooltipData = [
                `Price: $${parseFloat(candle.close || 0).toFixed(4)}`,
                `Open: $${parseFloat(candle.open || 0).toFixed(4)}`,
                `High: $${parseFloat(candle.high || 0).toFixed(4)}`,
                `Low: $${parseFloat(candle.low || 0).toFixed(4)}`,
                `Volume: ${formatVolume(parseFloat(candle.volume || 0))}`
              ]
              
              // Add indicator data if available
              if (value !== undefined && value !== null) {
                tooltipData.push(`Price: $${value.toFixed(4)}`)
              }
              
              return tooltipData
            }
            
            // For indicator datasets, only show the indicator value
            if (dataset && dataset.label && value !== undefined && value !== null) {
              if (dataset.label.includes('SMA')) {
                return [`SMA 20: $${value.toFixed(4)}`]
              } else if (dataset.label.includes('EMA 12')) {
                return [`EMA 12: $${value.toFixed(4)}`]
              } else if (dataset.label.includes('EMA 26')) {
                return [`EMA 26: $${value.toFixed(4)}`]
              } else if (dataset.label.includes('RSI')) {
                return [`RSI: ${value.toFixed(2)}`]
              } else if (dataset.label.includes('MACD Line')) {
                return [`MACD: ${value.toFixed(4)}`]
              } else if (dataset.label.includes('Signal Line')) {
                return [`Signal: ${value.toFixed(4)}`]
              } else if (dataset.label.includes('MACD Histogram')) {
                return [`Histogram: ${value.toFixed(4)}`]
              } else if (dataset.label.includes('Bollinger')) {
                      ...
                      callbacks: {
                        ...
                        label: (context) => {
                          try {
                            ...
                            console.error('Chart tooltip error:', error)
