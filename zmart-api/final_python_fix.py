...
# Define the final mapping for the remaining 4 services with correct paths
final_python_mappings = {
    'historical-data-service': 'src/services/historical_data_service.py',
    'pattern-recognition-service': 'src/services/pattern_recognition_service.py', 
    'scoring-service': 'src/services/scoring_service.py',
    'sentiment-analysis-service': 'src/services/sentiment_analysis_service.py'
}
...
for service_name, python_file in final_python_mappings.items():
    try:
        # Update the python_file_path field for this service
        cursor.execute('''
