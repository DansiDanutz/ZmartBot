    ...
    def scan_yaml_files(self) -> List[Dict]:
        ...
        for yaml_path in glob.glob("**/service.yaml", recursive=True):
            ...
            try:
                ...
                if content:
                    # Extract service name from nested structure: service.name or top-level service_name
                    service_name = content.get('service', {}).get('name') or content.get('service_name', 'unknown')
...
def main():
    ...
    parser.add_argument('--port', type=int, default=8895, help='Service port')
