...
class DatabaseService:
    ...
    def __init__(self, port: int = 8900):
        self.port = port
        # Use master database registry (adapter handles the compatibility)
...
<body data-theme="dark" style="background: #0f1419; color: #ffffff;">
    ...
    async def get_service_registry_data(self):
        ...
        try:
            # First, validate and cleanup ACTIVE services without ports
            await self.validate_and_cleanup_active_services()
    ...
    async def get_passport_registry_services(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE status = 'ACTIVE' AND port IS NOT NULL AND port != ''
            """)
    ...
    async def validate_and_cleanup_active_services(self):
        """Validate ACTIVE services and demote those without ports to DISCOVERY status"""
        try:
            passport_db_path = "/Users/dansidanutz/Desktop/ZmartBot/data/passport_registry.db"
            if not os.path.exists(passport_db_path):
                return
            ...
            # Find ACTIVE services without ports
            cursor.execute("""
            ...
            if invalid_services:
                logger.warning(f"🚨 Found {len(invalid_services)} ACTIVE services without ports - demoting to DISCOVERY")
                
                # Demote services without ports to DISCOVERY status
                cursor.execute("""
                    UPDATE passport_registry 
                    SET status = 'DISCOVERY', passport_id = NULL
                    WHERE status = 'ACTIVE' AND (port IS NULL OR port = '' OR port = 0)
                """)
                
                conn.commit()
                
                for service_name, port in invalid_services:
                    logger.warning(f"📉 Demoted {service_name} (port: {port}) from ACTIVE to DISCOVERY - FORBIDDEN: ACTIVE without port")
            
            conn.close()
            
        except Exception as e:
            logger.error(f"❌ Error validating ACTIVE services: {e}")
    ...
    def categorize_database(self, db_name: str, schema_info: dict) -> tuple:
        """Categorize database based on name and schema with comprehensive visual categories"""
        ...
        if any(term in name_lower for term in ['service_registry', 'master_database_registry']):
            return ("🏗️ System Core", "Core system registry databases", 4)
        elif any(term in name_lower for term in ['passport_registry', 'cert', 'discovery_registry']):
            return ("🔐 Service Management", "Service authentication and lifecycle", 3)
        
        # 📊 Trading & Market Data
        elif any(term in name_lower for term in ['symbol', 'market', 'trading', 'binance', 'kucoin']):
            return ("📊 Trading Data", "Market data and trading operations", 3)
        elif any(term in name_lower for term in ['risk', 'cowen', 'riskmetric']):
            return ("⚠️ Risk Management", "Risk analysis and management systems", 3)
        elif any(term in name_lower for term in ['prediction', 'ultimate']):
            return ("🎯 Predictions", "Market predictions and forecasting", 2)
        
        # 🤖 AI & Machine Learning
        elif any(term in name_lower for term in ['learning', 'pattern', 'grok', 'enhanced']):
            return ("🤖 AI Systems", "Machine learning and AI processing", 3)
        elif any(term in name_lower for term in ['cryptometer', 'cryptoverse']):
            return ("🔍 Data Mining", "Cryptocurrency data extraction", 2)
        
        # 🛡️ Security & Authentication  
        elif any(term in name_lower for term in ['auth', 'security', 'api_key', 'session']):
            return ("🛡️ Security", "Authentication and security systems", 3)
        
        # 📈 Analytics & Reporting
        elif any(term in name_lower for term in ['analytic', 'history', 'indicator', 'achievements']):
            return ("📈 Analytics", "Data analytics and reporting", 2)
        elif any(term in name_lower for term in ['log', 'servicelog', 'update_log']):
            return ("📝 Logging", "System logs and audit trails", 2)
        
        # 🔧 System Maintenance
        elif any(term in name_lower for term in ['backup', 'test_', 'temp_', 'tmp_']):
            return ("🔧 Maintenance", "System backups and testing", 1)
        elif any(term in name_lower for term in ['governance', 'failover', 'lifecycle']):
            return ("⚙️ Operations", "System operations and governance", 2)
        
        # 💾 Data Storage
        elif any(term in name_lower for term in ['data', 'storage', 'warehouse']):
            return ("💾 Data Storage", "Data warehousing and storage", 2)
        
        # 🔄 Integration & Services
        elif any(term in name_lower for term in ['discovery', 'workflow', 'recommendation']):
            return ("🔄 Integration", "Service discovery and integration", 2)
        elif any(term in name_lower for term in ['winner', 'impact']):
            return ("🏆 Optimization", "System optimization and winners", 1)
        
        # 🔌 Infrastructure
        elif any(term in name_lower for term in ['port', 'registry']):
            return ("🔌 Infrastructure", "System infrastructure and ports", 2)
        
        # ❓ Unategorized
        else:
            return ("❓ Other", "Uncategorized database", 1)
    
    def calculate_health_score(self, size_bytes: int, table_count: int, record_count: int) -> float:
        """Calculate database health score based on various factors with improved accuracy"""
        score = 70.0  # Start with a base score
        
        # Size-based health (more nuanced scoring)
        if size_bytes == 0:
            score = 20.0  # Critical - empty database
        elif size_bytes < 100:  # Very small files (likely corrupt or minimal)
            score = 35.0
        elif size_bytes < 1024:  # Small databases
            score += 10.0  # Small but functional
        elif size_bytes < 50_000_000:  # Normal size (< 50MB)
            score += 20.0  # Healthy size
        elif size_bytes < 500_000_000:  # Large but manageable (< 500MB)
            score += 15.0  # Large but OK
        else:  # Very large databases (> 500MB)
            score += 5.0  # Large databases need optimization
        
        # Table structure health
        if table_count == 0:
            score = min(score, 30.0)  # No tables is critical
        elif table_count == 1:
            score += 15.0  # Single-purpose database (good design)
        elif 2 <= table_count <= 10:
            score += 10.0  # Well-structured database
        elif 11 <= table_count <= 50:
            score += 5.0  # Complex but manageable
        else:
            score += 0.0  # Very complex database
        
        # Data population health
        if table_count > 0:
            if record_count == 0:
                score -= 25.0  # Tables without data
            elif record_count < 10:
                score += 5.0  # Minimal data
            elif record_count < 1000:
                score += 10.0  # Good amount of data
            elif record_count < 100000:
                score += 15.0  # Rich dataset
            else:
                score += 10.0  # Very large dataset
        
        # Data density check (records per table)
        if table_count > 0 and record_count > 0:
            density = record_count / table_count
            if density > 100:  # Good data density
                score += 5.0
            elif density < 1:  # Poor data density
                score -= 10.0
    ...
    def calculate_enhanced_health_score(self, db_path: str, table_count: int, record_count: int, size_bytes: int) -> float:
        """Calculate enhanced health score based on database connectivity and system integration"""
        base_score = self.calculate_health_score(size_bytes, table_count, record_count)
        
        # Additional health factors for production readiness
        db_path_obj = Path(db_path)
        db_name = db_path_obj.name.lower()
        
        # Critical system databases get bonus points
        if any(term in db_name for term in ['service_registry', 'passport', 'cert', 'master']):
            base_score += 10.0  # Core system databases
        elif any(term in db_name for term in ['backup', 'history', 'log']):
            base_score += 5.0   # Support databases
        
        # Database age and modification recency
        try:
            if db_path_obj.exists():
                modification_age = (datetime.now() - datetime.fromtimestamp(db_path_obj.stat().st_mtime)).days
                if modification_age <= 1:
                    base_score += 5.0  # Recently modified (active)
                elif modification_age <= 7:
                    base_score += 2.0  # Recently modified
                elif modification_age > 90:
                    base_score -= 10.0  # Stale database
        except:
            pass
        
        # Database naming convention bonus
        if any(pattern in db_name for pattern in ['.db', '_registry', '_data', '_service']):
            base_score += 3.0  # Good naming convention
        
        # Empty or test databases penalty
        if any(term in db_name for term in ['test_', 'temp_', 'tmp_']) and size_bytes < 10000:
            base_score -= 15.0  # Test databases should be cleaned up
        
        return max(15.0, min(100.0, base_score))
    ...
    def create_app(self) -> FastAPI:
        ...
        async def get_system_overview():
            ...
            try:
                ...
                # Database health and status stats
                cursor.execute("""
                    SELECT 
                        COUNT(*) as total_databases,
                        SUM(CASE WHEN health_score < 70 OR status = 'empty' THEN 1 ELSE 0 END) as problem_databases,
                        SUM(CASE WHEN health_score >= 70 AND status != 'empty' THEN 1 ELSE 0 END) as working_databases
                    FROM database_registry
                """)
                
                database_health = cursor.fetchone()
                ...
                return {
                    ...
                    "database_status": {
                        "total_databases": database_health[0] or 0,
                        "problem_databases": database_health[1] or 0,
                        "working_databases": database_health[2] or 0
                    },
                    "system_resources": {
        ...
        @app.post("/api/services/validate-active")
        async def validate_active_services():
            """Validate ACTIVE services and demote those without ports"""
            try:
                await self.validate_and_cleanup_active_services()
                return {"status": "success", "message": "ACTIVE services validation completed"}
            except Exception as e:
                logger.error(f"❌ Error validating ACTIVE services: {e}")
        ...
        @app.post("/api/operations/analyze/{db_name}")
        async def analyze_database_tables(db_name: str):
            """Analyze database tables structure and statistics"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                    if isinstance(db, dict) and db.get('name') == db_name:
                        db_info = db
                        break
                    elif isinstance(db, str) and db == db_name:
                        # Create a basic db_info structure for string entries
                        db_info = {'name': db_name, 'path': db}
        ...
        @app.post("/api/operations/optimize/{db_name}")
        async def optimize_database(db_name: str):
            """Optimize database performance - READ ONLY, NO DELETION"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                ...
                # Run VACUUM and ANALYZE (safe operations)
                ...
                optimized_size = (page_count * page_size) / (1024 * 1024)  # MB
                
                conn.close()
                
                return {
                    "status": "success",
                    "message": "Database optimized successfully - No data deleted",
                    "data": {
                        "database": db_name,
        ...
        @app.post("/api/operations/backup/{db_name}")
        ...
        @app.get("/api/operations/schema/{db_name}")
        async def get_database_schema(db_name: str):
            """Get complete database schema - READ ONLY"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                ...
                cursor.execute("SELECT name, type FROM sqlite_master WHERE type IN ('table', 'index', 'view')")
                ...
                schema_info = {
                    ...
                    "schema_sql": [stmt[0] for stmt in schema_statements]
                }
                
                for obj in objects:
                    schema_info["objects"][f"{obj[1]}s"].append(obj[0])
        ...
        @app.post("/api/operations/query/{db_name}")
        async def execute_database_query(db_name: str, query: dict):
            """Execute read-only SQL query - SELECT ONLY"""
            try:
                sql_query = query.get("sql", "").strip()
                
                # Security: Only allow SELECT queries - NO DELETION
                if not sql_query.upper().startswith("SELECT"):
                    raise HTTPException(status_code=400, detail="Only SELECT queries are allowed - NO DELETION")
                ...
                return {
                    ...
                    "data": {
                        ...
                        "results": result_data[:100]  # Limit to 100 rows for safety
                    }
                }
        ...
        @app.get("/api/operations/export/{db_name}")
        async def export_database_data(db_name: str, format: str = "json"):
            """Export database data - READ ONLY"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                ...
                return {
                    "status": "success",
                    "message": f"Database exported in {format} format - READ ONLY",
                    "data": export_data
                }
        ...
        @app.get("/api/operations/logs/{db_name}")
        async def get_database_logs(db_name: str):
            """Get database operation logs - READ ONLY"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                ...
                log_file = Path("database_service.log")
                if log_file.exists():
                ...
                return {
                    ...
                    "data": {
                        ...
                        "file_info": {
                            ...
                            "created": datetime.fromtimestamp(db_stat.st_ctime).isoformat()
                        },
                        "recent_logs": log_entries,
                        "health_score": db_info.get('health_score', 0)
                    }
                }
        ...
        @app.post("/api/operations/repair/{db_name}")
        async def repair_database(db_name: str):
            """Repair database corruption - NO DELETION, REPAIR ONLY"""
            try:
                # Find database in the monitored databases
                db_info = None
                for db in self.databases:
                ...
                # Create repair backup first
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                repair_backup = f"{db_path}.repair_backup_{timestamp}"
                import shutil
                shutil.copy2(db_path, repair_backup)
                ...
                # If integrity issues found, try to repair
                if integrity_result[0][0] != "ok":
                    # Try VACUUM to repair (safe, no data deletion)
                    cursor.execute("VACUUM")
                    repair_actions.append("VACUUM (structure repair)")
                    
                    # Reindex all indexes (safe, no data deletion)
                    cursor.execute("REINDEX")
                    repair_actions.append("REINDEX (index rebuild)")
                    
                    # Run integrity check again
                    cursor.execute("PRAGMA integrity_check")
                    final_result = cursor.fetchall()
                    
                    if final_result[0][0] == "ok":
                        repair_actions.append("Integrity restored - No data lost")
                    else:
                        repair_actions.append("Manual intervention required - Data preserved")
                
                # Update health score
                new_health = 95 if integrity_result[0][0] == "ok" else 60
                
                conn.close()
                
                return {
                    "status": "success",
                    "message": "Database repair completed - NO DATA DELETED",
                    "data": {
                        "database": db_name,
                        "repair_backup": repair_backup,
                        "integrity_status": integrity_result[0][0],
...
def main():
    ...
    parser.add_argument('--port', default=8900, type=int, help='Port to bind to')
