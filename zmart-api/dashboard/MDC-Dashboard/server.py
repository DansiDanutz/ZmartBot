...
Flask backend for the MDC Dashboard running on localhost:8090/MDC-dashboard
"""
...
class MDCDashboardServer:
    ...
    def __init__(self, project_root: str = None, port: int = 8090, mdc_path: str = None):
        ...
        # 💎 Diamond Discovery System - Using ALL MDC files
        mdc_files_count = len(list(self.mdc_dir.glob("*.mdc")))
        logger.info(f"💎 DIAMOND SYSTEM ACTIVATED: Using {mdc_files_count} MDC files for 3-phase connection discovery")
    ...
    def setup_routes(self):
        ...
        @self.app.route('/api/system/clear-cache', methods=['POST'])
        def clear_system_cache():
            """Clear system cache to force fresh data"""
            self.cache = {
                "system_status": None,
                "mdc_files": None,
                "connection_health": "connected",
            ...
            logger.info("🔧 System cache cleared - next request will fetch fresh data")
            return jsonify({
                "success": True,
        ...
        def get_system_status():
            ...
            try:
                ...
                if force_refresh:
                    ...
                    # Skip cache check and get fresh data
                else:
                    # 🚀 Check cache first to prevent showing 0 values
    ...
    def perform_comprehensive_scan(self, scan_path: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_info in mdc_files:
                ...
                active_count += len([line for line in content.split('\n') if '✅' in line and 'ACTIVE' in line])
                potential_count += len([line for line in content.split('\n') if '⏳' in line and 'POTENTIAL' in line])
                priority_count += len([line for line in content.split('\n') if '🔥' in line and 'PRIORITY' in line])
...
"""
    ...
    def is_service_file(self, py_file: Path) -> bool:
        ...
        try:
            # Skip test files, __init__.py, and common utility files
            if any(skip in py_file.name.lower() for skip in ['test_', '_test', '__init__', 'config', 'utils', 'helper']):
                return False
...
- **Update Policy**: Real-time updates during workflow execution"""
    ...
    def get_system_health(self) -> Dict[str, Any]:
        """Get system health status - Use local counting logic for accurate connections"""
        try:
            # 🔧 FIXED: Always use local counting logic for accurate connection count
            # The orchestration service has incorrect connection counting (showing 72 vs actual 1,388)
            logger.info("🔧 Using local connection counting logic (orchestration service bypass)")
            return self.get_default_system_status()
            
            # Original logic disabled to prevent incorrect connection counts:
            # response = requests.get(f"{self.orchestration_url}/status", timeout=5)
            # if response.status_code == 200:
            #     return response.json()
    ...
    def get_default_system_status(self) -> Dict[str, Any]:
        """Get default system status - Direct file counting for accurate connections"""
        # 🔧 FIXED: Direct file reading for accurate connection count
        # The scan_mdc_files() method removes content after processing, causing 0 connections
        ...
        # Direct file reading from MDC directory
            ...
            for mdc_file in self.mdc_dir.glob("*.mdc"):
                if mdc_file.is_file():
                    ...
                    try:
                        ...
                        logger.error(f"Error reading {mdc_file.name} for connection count: {e}")
        
        # 🔧 FIXED: Get actual service counts from orchestration service
        actual_services_count = 0
        passport_services_count = 0
        
        try:
            response = requests.get("http://localhost:8002/api/orchestration/services", timeout=5)
            if response.status_code == 200:
                services = response.json()
                actual_services_count = len(services)
                passport_services_count = sum(1 for service in services if service.get('passport_id') is not None)
                logger.info(f"🔧 Service counts: {actual_services_count} registered services ({passport_services_count} with passport)")
            else:
                logger.warning(f"Could not get service counts from orchestration service: {response.status_code}")
        except Exception as e:
            logger.warning(f"Could not get service counts from orchestration service: {e}")

        logger.info(f"🔧 Connection count calculated: {total_connections} total ({active_connections} active + {potential_connections} potential + {priority_connections} priority) across {total_files} MDC files")
        ...
        return {
            ...
            "total_services": actual_services_count,
            "registered_services": actual_services_count,
            "passport_services": passport_services_count,
