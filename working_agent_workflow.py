#!/usr/bin/env python3
"""
Working Agent Workflow for Professional Trading Analysis
Uses actual Airtable field names that exist in the table
"""
...
"""
    ...
    def _analyze_long_term_ratios(self, data: Dict) -> str:
        ...
        return f"""# Long-term Liquidation Ratios Analysis

## Market Overview
**Analysis Type:** Long-term Liquidation Ratios
**Analysis Date:** {datetime.now().strftime('%B %d, %Y')}

## Key Findings
- **Institutional Analysis:** {data.get('institutional', 'N/A')}
- **Trend Analysis:** {data.get('trend', 'N/A')}
- **Risk Assessment:** {data.get('risk', 'Medium')}
- **Confidence Level:** {data.get('confidence', 75)}%

## Technical Summary
Long-term ratios analysis completed with institutional and trend assessment.
"""
    
    def _analyze_short_term_ratios(self, data: Dict) -> str:
        """Professional short-term ratios analysis"""
        return f"""# Short-term Liquidation Ratios Analysis

## Market Overview
**Analysis Type:** Short-term Liquidation Ratios
**Analysis Date:** {datetime.now().strftime('%B %d, %Y')}

## Key Findings
- **Immediate Pressure:** {data.get('pressure', 'N/A')}
- **Market Sentiment:** {data.get('sentiment', 'Neutral')}
- **Risk Assessment:** {data.get('risk', 'Medium')}
- **Confidence Level:** {data.get('confidence', 70)}%

## Technical Summary
Short-term ratios analysis completed with immediate pressure assessment.
"""
...
**Available Analyses:** {analysis_count}
...
{chr(10).join([f"📊 **{analysis_type.upper()} Analysis:** Available" for analysis_type in all_analyses.keys()])}

## Win Rate Summary
**24H-48H:** Long {win_rates['24h48h']['long']:.0f}%, Short {win_rates['24h48h']['short']:.0f}%
**7 Days:** Long {win_rates['7days']['long']:.0f}%, Short {win_rates['7days']['short']:.0f}%
**1 Month:** Long {win_rates['1month']['long']:.0f}%, Short {win_rates['1month']['short']:.0f}%
**Score:** {max(win_rates['24h48h']['long'], win_rates['24h48h']['short']):.0f},{max(win_rates['7days']['long'], win_rates['7days']['short']):.0f},{max(win_rates['1month']['long'], win_rates['1month']['short']):.0f}
...
"""
    ...
    async def process_new_image(self, symbol: str, image_type: str, image_data: Dict) -> bool:
        ...
        try:
            ...
            # 3. Map image types to Airtable field names
            field_mapping = {
            ...
            if existing_row:
                # Update existing row
                existing_fields = existing_row['fields']
                
                # Update the specific analysis field
                existing_fields[airtable_field] = analysis
                
                # 4. Calculate new win rates
                win_rates = self.calculate_win_rates(symbol, existing_fields)
                
                # 5. Generate comprehensive result
                timestamp = datetime.now().isoformat()
                comprehensive_result = self.generate_comprehensive_result(symbol, existing_fields, win_rates, timestamp)
                existing_fields['Result'] = comprehensive_result
                
                # 6. Update win rate fields with proper format
                existing_fields['24h48h'] = self.format_win_rate_field(
                    win_rates['24h48h']['long'], 
                    win_rates['24h48h']['short']
                )
                existing_fields['7days'] = self.format_win_rate_field(
                    win_rates['7days']['long'], 
                    win_rates['7days']['short']
                )
                existing_fields['1Month'] = self.format_win_rate_field(
                ...
                existing_fields['Score(24h48h_7Days_1Month)'] = score_field
                
                # 8. Update timestamp
                existing_fields['Last_update'] = timestamp
                
                # 9. Update Airtable
                success = await self.update_airtable_row(existing_row['id'], existing_fields)
                ...
                # Create new row
                analyses = {airtable_field: analysis}
                win_rates = self.calculate_win_rates(symbol, analyses)
                timestamp = datetime.now().isoformat()
                comprehensive_result = self.generate_comprehensive_result(symbol, analyses, win_rates, timestamp)
                
                record = {
                    "fields": {
                        "Symbol": symbol,
                        airtable_field: analysis,
                ...
                success = await self.create_new_symbol_row(record)
                return success
    ...
    async def create_new_symbol_row(self, record: Dict) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    result = response.json()
                    logger.info(f"✅ Created new row for {record['fields']['Symbol']}")
...
async def main():
    """Test the working agent workflow"""
    
    workflow = WorkingAgentWorkflow()
    ...
    print("🚀 Testing Working Agent Workflow...")
    print("=" * 60)
    ...
    print("\n✅ Working Agent Workflow Test Complete!")
