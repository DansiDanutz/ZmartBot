#!/usr/bin/env python3
"""
ETH USDT Comprehensive Analysis with Live Cryptometer API Data
...
class ComprehensiveETHCryptometerAnalyzer:
    def __init__(self):
    ...
    def collect_all_cryptometer_data(self):
        """Collect data from all 18 Cryptometer endpoints for ETH"""
        print("Starting Comprehensive ETH USDT Cryptometer API Collection...")
        ...
        endpoints = [
            ...
            },
            {
                ...
                'params': {'symbol': 'ETH', 'timeframe': '1h', 'exchange_type': 'spot'},
            ...
            },
            {
                'name': 'whale_trades',
                'endpoint': '/xtrades/',
                'params': {'e': self.exchange, 'symbol': 'eth'},
            ...
            },
            {
                ...
                'params': {'symbol': 'ETH'},
                'description': 'AI Analysis - ETH-specific AI insights'
            }
        ]
                ...
                if endpoint_name == 'trend_indicator':
                    ...
                    if trend_data:
                        ...
                        for item in trend_data:
                            if 'ETH' in item.get('symbol', '').upper():
                                eth_trend = item
                                break
                        
                        if eth_trend:
                            metrics['trend_indicators'] = {
                                'trend_score': float(eth_trend.get('trend_score', 50)),
                                'buy_pressure': float(eth_trend.get('buy_pressure', 50)),
                                'sell_pressure': float(eth_trend.get('sell_pressure', 50)),
                                'momentum': float(eth_trend.get('momentum', 50))
                            }
                    ...
                    if eth_liquidity:
                        metrics['volume_indicators']['liquidity_analysis'] = {
                            'eth_inflow': float(eth_liquidity.get('inflow', 0)),
                            'eth_outflow': float(eth_liquidity.get('outflow', 0)),
                            'eth_netflow': float(eth_liquidity.get('netflow', 0))
                        }
                    ...
                    eth_ai_data = [item for item in ai_data if 'ETH' in item.get('symbol', '')]
                    
                    if eth_ai_data:
                        ai_item = eth_ai_data[0]
                    ...
                    eth_vol = [item for item in vol_data if 'ETH' in item.get('symbol', '')]
    ...
    def calculate_advanced_composite_scores(self, metrics):
        ...
        scores = {
            'methodology': 'Advanced Multi-Source Composite Scoring for ETH',
        ...
        if 'volatility' in metrics['technical_indicators']:
            ...
            # ETH typically benefits from moderate volatility
            if 25 <= vol_score <= 75:
        ...
        if metrics['trend_indicators']:
            ...
            eth_adjusted_trend = trend_score * 1.05  # ETH typically shows stronger trends
            technical_factors.extend([eth_adjusted_trend, buy_pressure, 100 - sell_pressure])
        ...
        if 'ls_ratio' in metrics['sentiment_indicators']:
            ...
            # ETH sentiment analysis (less contrarian than smaller caps)
            if long_pct > 80:  # Extreme long positioning
                sentiment_factors.append(35)  # Moderately bearish
            elif long_pct < 20:  # Extreme short positioning
                sentiment_factors.append(75)  # Bullish contrarian signal
            else:
                sentiment_factors.append(50 + (long_pct - 50) * 0.6)  # ETH follows sentiment more
        ...
        if 'analysis' in metrics['ai_indicators']:
            ...
            sentiment_factors.append(ai_sentiment * 1.1)  # AI sentiment more reliable for ETH
        ...
        if 'flow_analysis' in metrics['volume_indicators']:
            ...
            # ETH volume analysis (higher volume = more significant)
            if net_flow > 0:
                flow_score = min(80, 50 + (net_flow / 5000000) * 15)  # Scale by 5M for ETH
            else:
                flow_score = max(20, 50 + (net_flow / 5000000) * 15)
            
            volume_factors.append(flow_score)
        
        if 'liquidity_analysis' in metrics['volume_indicators']:
            eth_netflow = metrics['volume_indicators']['liquidity_analysis']['eth_netflow']
            if eth_netflow > 0:
                liquidity_score = min(80, 50 + abs(eth_netflow) / 500000 * 15)  # ETH-specific scaling
            else:
                liquidity_score = max(20, 50 - abs(eth_netflow) / 500000 * 15)
        ...
        if 'large_trades' in metrics['volume_indicators']:
            ...
            if buy_sell_ratio > 1:
                large_trade_score = min(80, 50 + (buy_sell_ratio - 1) * 30)  # ETH institutional activity more impactful
            else:
                large_trade_score = max(20, 50 - (1 - buy_sell_ratio) * 30)
        ...
        if metrics['liquidation_indicators']:
            ...
            if total_longs + total_shorts > 0:
                ...
                # ETH liquidation analysis (more stable than smaller caps)
                if long_liq_pct > 75:
                    liquidation_factors.append(60)  # Moderate oversold signal
                elif long_liq_pct < 25:
                    liquidation_factors.append(40)  # Moderate overbought signal
                else:
        ...
        # Short position score (ETH-specific adjustments)
        short_technical = 100 - scores['components']['technical_analysis']['score']
        short_sentiment = scores['components']['sentiment_analysis']['score'] * 0.7  # ETH sentiment less contrarian
        ...
        scores['final_scores'] = {
            'long_score': max(25, min(85, long_score)),  # ETH can have higher confidence
            'short_score': max(15, min(75, short_score)),  # ETH shorts generally riskier long-term
    ...
    def calculate_enhanced_win_rates(self, composite_scores, metrics):
        ...
        def advanced_score_to_win_rate(score, timeframe, position, confidence):
            """Advanced win rate calculation with ETH-specific adjustments"""
            # Base conversion with ETH-specific characteristics
            base_rate = score * 0.9  # ETH generally more predictable than smaller caps
            
            # Confidence adjustment
            confidence_factor = 0.85 + (confidence / 100) * 0.3  # 0.85 to 1.15 range
            
            # Timeframe adjustments for ETH
            timeframe_factors = {
                '24-48h': 0.9,   # ETH can be volatile short-term
                '7d': 1.0,       # Balanced
                '1m': 1.2        # ETH shows strong long-term trends
            }
            
            # Position-specific adjustments for ETH
            if position == 'long':
                # ETH has strong long-term fundamentals
                if timeframe == '1m':
                    position_factor = 1.15  # Strong long-term uptrend
                elif timeframe == '7d':
                    position_factor = 1.05  # Moderate advantage
                else:
                    position_factor = 1.0
            else:  # short
                # ETH shorts are challenging due to strong fundamentals
                if timeframe == '1m':
                    position_factor = 0.8   # Difficult to short ETH long-term
                elif timeframe == '7d':
                    position_factor = 0.9   # Moderate difficulty
                else:
                    position_factor = 0.95  # Short-term shorts more viable
            
            # Data quality adjustment
            data_sources_count = len(metrics['data_sources'])
            data_quality_factor = 0.9 + (data_sources_count / 18) * 0.2  # 0.9 to 1.1 range
            
            # ETH market position adjustment (major crypto)
            eth_factor = 1.05  # ETH is more predictable due to size and liquidity
            
            # Calculate final win rate
            win_rate = (base_rate * confidence_factor * 
                       timeframe_factors[timeframe] * 
                       position_factor * data_quality_factor * eth_factor)
            
            # Cap between realistic bounds for ETH
        ...
        win_rates = {
            'methodology': 'Enhanced Multi-Factor Win Rate Calculation for ETH',
            'confidence_level': confidence,
            'data_quality_score': len(metrics['data_sources']) / 18 * 100,
            'eth_specific_adjustments': 'Applied major cryptocurrency factors',
            'timeframes': {
                '24-48h': {
    ...
    def generate_comprehensive_analysis(self):
        """Generate the most comprehensive ETH analysis possible"""
        print("\n" + "="*60)
        print("GENERATING COMPREHENSIVE ETH ANALYSIS")
        print("="*60)
        
        # Step 1: Collect all Cryptometer data
        print("Step 1: Collecting live Cryptometer API data for ETH...")
        ...
        analysis = {
            'analysis_metadata': {
                'timestamp': datetime.now().isoformat(),
                'symbol': self.symbol,
                'analysis_type': 'Comprehensive Multi-Source Professional Analysis - ETH USDT',
                'data_sources_count': len(metrics['data_sources']),
...
def main():
    ...
    print("Starting Most Comprehensive ETH USDT Analysis...")
    print("Using Live Cryptometer API + Advanced Analytics")
    print("="*60)
    
    # Initialize analyzer
    analyzer = ComprehensiveETHCryptometerAnalyzer()
    ...
    results_file = f"/home/ubuntu/eth_comprehensive_analysis_{timestamp}.json"
    
    with open(results_file, 'w') as f:
        json.dump(analysis, f, indent=2, default=str)
    
    print(f"\n" + "="*60)
    print("COMPREHENSIVE ETH ANALYSIS COMPLETED")
    print("="*60)
