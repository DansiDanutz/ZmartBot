...
/**
 * ZMARTY SCHEMA DEPLOYMENT SCRIPT
 * Deploys the complete database schema to your ZmartyBrain Supabase project
 */
...
console.log('🚀 Starting ZmartyBrain Schema Deployment...');
console.log(`📡 Target: ${process.env.SUPABASE_URL}`);

const schemas = [
  {
    name: 'Extensions and Core Setup',
    sql: `
  ...
  {
    name: 'Historical Patterns System',
    sql: `
  ...
  {
    name: 'Payments and Credits System',
    sql: `
      -- Credit purchases
...
async function deploySchema() {
  for (const schema of schemas) {
    try {
      console.log(`\n🔧 Deploying: ${schema.name}...`);

      const { error } = await supabase.rpc('exec_sql', {
        sql_statement: schema.sql
      });

      if (error) {
        console.error(`❌ Error deploying ${schema.name}:`, error);
      } else {
        console.log(`✅ Successfully deployed: ${schema.name}`);
      }
    } catch (err) {
      console.error(`❌ Exception deploying ${schema.name}:`, err.message);
    }
  }
  ...
  try {
    ...
    if (error) {
      console.log('⚠️  Verification failed - checking table existence...');
      ...
      const { data: tables, error: tableError } = await supabase
        .rpc('exec_sql', {
          sql_statement: `
            ...
            AND table_name IN ('users', 'brain_knowledge', 'invitations', 'trigger_events')
            ORDER BY table_name;
          `
        });
      ...
      console.log('✅ Schema deployment successful! Users table is accessible.');
    }
  } catch (err) {
    console.error('❌ Verification error:', err.message);
  }

  console.log('\n🎉 ZmartyBrain schema deployment complete!');
  console.log('🔗 Check your database: https://supabase.com/dashboard/project/xhskmqsgtdhehzlvtuns/editor');
}
