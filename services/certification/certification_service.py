...
class CertificationService:
    ...
    def validate_port_assignment(self, service: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            port_registry_path = self.project_root / "zmart-api" / "port_registry.db"
    ...
    def validate_newservice_workflow(self, service_name: str) -> bool:
        ...
        service_yaml_path = self.project_root / "services" / service_name / "service.yaml"
        ...
        zmart_api_paths = [
            self.project_root / "zmart-api" / service_name / "service.yaml",
            self.project_root / "zmart-api" / service_name.replace('-', '_') / "service.yaml",
            self.project_root / "zmart-api" / service_name.replace('_', '-') / "service.yaml"
        ]
        
        # Add mapped directory names
        if service_name in service_mappings:
            zmart_api_paths.append(self.project_root / "zmart-api" / service_mappings[service_name] / "service.yaml")
        ...
        dashboard_paths = [
            self.project_root / "zmart-api" / "dashboard" / f"{service_name}" / "service.yaml",
            self.project_root / "zmart-api" / "dashboard" / f"{service_name.upper()}" / "service.yaml"
        ]
