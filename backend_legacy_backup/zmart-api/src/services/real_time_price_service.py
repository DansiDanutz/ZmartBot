...
class RealTimePriceService:
    ...
    async def _ensure_initialized(self):
        ...
        if not self._initialized:
            ...
            await self.binance_service.__aenter__()
    ...
    async def get_real_time_price(self, symbol: str) -> Optional[RealTimePrice]:
        ...
        try:
            ...
            # 1. Try Binance FIRST (PRIMARY for all price data)
            try:
                # Convert symbol to Binance format (standard format)
                binance_symbol = to_binance(symbol)
                if self.binance_service:
                    binance_price = await self.binance_service.get_price(binance_symbol)
                    if binance_price:
            ...
            # 2. Try KuCoin (backup for price, but PRIMARY for trading)
            try:
                # Convert symbol to KuCoin format for futures
                kucoin_symbol = to_kucoin(symbol, is_futures=True)
                if self.kucoin_service:
                    kucoin_data = await self.kucoin_service.get_market_data(kucoin_symbol)
            ...
            # 3. Try Cryptometer as last resort
            try:
                # Cryptometer typically uses standard format
                standard_symbol = to_standard(symbol)
                if self.cryptometer_service:
                    crypto_data = await self.cryptometer_service.collect_symbol_data(standard_symbol)
            ...
            # NO PRICES? NO MOCK! Return None or raise error
            if not prices:
                logger.error(f"NO REAL PRICE DATA AVAILABLE for {symbol}")
                raise ValueError(f"Cannot get real price for {symbol} - all sources failed")
    ...
    async def get_technical_data(self, symbol: str) -> Optional[TechnicalData]:
        ...
        try:
            ...
            if not data or 'error' in data:
                logger.error(f"No technical data available for {symbol}")
                return None
