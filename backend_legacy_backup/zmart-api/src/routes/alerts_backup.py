...
async def get_symbol_technical_analysis(symbol: str):
    ...
    try:
        ...
        # Get historical klines for different timeframes
        timeframes_data = {}
        ...
        for tf in timeframes:
            try:
                ...
                limit_map = {
                    "15m": 200,  # More data for shorter timeframes
                    "1h": 100,
                    "4h": 100,
                    "1d": 100
                }
                ...
                timeframes_data[tf] = prices
                
            except Exception as e:
                logger.warning(f"Failed to get {tf} data for {symbol}: {e}")
                timeframes_data[tf] = []
        
        # Use 1h data as primary for main indicators
        prices = timeframes_data.get("1h", [])
        ...
        analysis = {
            ...
            # Moving Averages
            "ema_12": ta_service.calculate_ema(prices, 12)[-1] if len(prices) >= 12 else None,
            "ema_26": ta_service.calculate_ema(prices, 26)[-1] if len(prices) >= 26 else None,
            "sma_20": sum(prices[-20:]) / 20 if len(prices) >= 20 else None,
            "sma_50": sum(prices[-50:]) / 50 if len(prices) >= 50 else None,
            
            # RSI
            "rsi": ta_service.calculate_rsi(prices, 14)[-1] if len(prices) >= 15 else None,
            "rsi_signal": ta_service.detect_rsi_signals(prices),
            
            # MACD (will be populated from database)
            "macd": None,  # Will be populated from database
            "macd_signal": ta_service.detect_macd_signals(prices),
            "macd_data": {},
            
            # Bollinger Bands (1h timeframe)
            "bollinger_bands": None,  # Will be populated from database
            
            # Bollinger Bands for all timeframes
            "bollinger_bands_timeframes": {},
            
            # Cross Patterns
            "golden_cross": ta_service.detect_golden_cross(prices),
            "death_cross": ta_service.detect_death_cross(prices),
            
            # Alert Status
            "alerts": []
        }
        
        # Get database connection for Bollinger Bands and alerts
        import sqlite3
        db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'my_symbols_v2.db')
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # Try to get Bollinger Bands from database first, then calculate if needed
        try:
            cursor.execute("""
                SELECT timeframe, sma, upper_band, lower_band, bandwidth, position
                FROM bollinger_bands 
                WHERE symbol = ?
                ORDER BY timeframe
            """, (symbol,))
            
            db_bollinger_data = cursor.fetchall()
            
            if db_bollinger_data:
                # Use database data
                for row in db_bollinger_data:
                    tf, sma, upper_band, lower_band, bandwidth, position = row
                ...
                # Calculate Bollinger Bands for all timeframes if not in database
                for tf in timeframes:
                    tf_prices = timeframes_data.get(tf, [])
                    if len(tf_prices) >= 20:
                        sma, upper_band, lower_band = ta_service.calculate_bollinger_bands(tf_prices)
                        if sma and upper_band and lower_band:
                            analysis["bollinger_bands_timeframes"][tf] = {
                                ...
                                "bandwidth": ((upper_band[-1] - lower_band[-1]) / sma[-1] * 100) if sma and upper_band and lower_band else None,
                                "position": ((current_price - lower_band[-1]) / (upper_band[-1] - lower_band[-1]) * 100) if upper_band and lower_band else None
                            }
                        else:
                            analysis["bollinger_bands_timeframes"][tf] = None
                    else:
                        analysis["bollinger_bands_timeframes"][tf] = None
        except Exception as e:
        ...
        # Try to get Momentum Indicators data from database
        try:
            cursor.execute("""
                SELECT timeframe, roc_value, roc_signal, roc_strength, roc_divergence,
                       mom_value, mom_signal, mom_strength, mom_divergence,
            ...
            if db_momentum_indicators_data:
                ...
                for row in db_momentum_indicators_data:
                    tf, roc_value, roc_signal, roc_strength, roc_divergence, mom_value, mom_signal, mom_strength, mom_divergence, momentum_status, momentum_strength, trend_alignment, overbought_oversold_status, volume_confirmation = row
                    analysis["momentum_indicators_data"] = analysis.get("momentum_indicators_data", {})
                    analysis["momentum_indicators_data"][tf] = {
                        "roc_value": roc_value,
                ...
                # No Momentum Indicators data in database - skip calculation for now
                analysis["momentum_indicators_data"] = {}
        except Exception as e:
            logger.warning(f"Error reading Momentum Indicators from database for {symbol}: {e}")
            # No fallback calculation - skip Momentum Indicators data
            analysis["momentum_indicators_data"] = {}

        # Try to get Price Channels data from database
        try:
            cursor.execute("""
                SELECT timeframe, upper_channel, middle_channel, lower_channel,
                       channel_width, channel_position, breakout_direction, breakout_strength,
            ...
            if db_price_channels_data:
                ...
                for row in db_price_channels_data:
                    tf, upper_channel, middle_channel, lower_channel, channel_width, channel_position, breakout_direction, breakout_strength, channel_trend, trend_strength, volatility_status, volatility_strength, momentum_status, momentum_strength, volume_confirmation = row
                    analysis["price_channels_data"] = analysis.get("price_channels_data", {})
                    analysis["price_channels_data"][tf] = {
                ...
                # No Price Channels data in database - skip calculation for now
                analysis["price_channels_data"] = {}
        except Exception as e:
            logger.warning(f"Error reading Price Channels from database for {symbol}: {e}")
            # No fallback calculation - skip Price Channels data
            analysis["price_channels_data"] = {}

        # Try to get Support/Resistance Levels data from database
        try:
            cursor.execute("""
                SELECT timeframe, support_level_1, support_level_2, support_level_3,
                       resistance_level_1, resistance_level_2, resistance_level_3,
            ...
            if db_support_resistance_data:
                ...
                for row in db_support_resistance_data:
                    tf, support_level_1, support_level_2, support_level_3, resistance_level_1, resistance_level_2, resistance_level_3, price_position, nearest_support, nearest_resistance, support_strength, resistance_strength, breakout_potential, breakout_direction, breakout_strength, volume_confirmation = row
                    analysis["support_resistance_data"] = analysis.get("support_resistance_data", {})
                    analysis["support_resistance_data"][tf] = {
                ...
                # No Support/Resistance Levels data in database - skip calculation for now
                analysis["support_resistance_data"] = {}
        except Exception as e:
            logger.warning(f"Error reading Support/Resistance Levels from database for {symbol}: {e}")
            # No fallback calculation - skip Support/Resistance Levels data
            analysis["support_resistance_data"] = {}

        # Try to get Moving Average Convergence data from database
        try:
            cursor.execute("""
                SELECT timeframe, convergence_status, convergence_strength, ma_alignment,
                       alignment_strength, golden_cross_detected, death_cross_detected,
            ...
            if db_ma_convergence_data:
                ...
                for row in db_ma_convergence_data:
                    tf, convergence_status, convergence_strength, ma_alignment, alignment_strength, golden_cross_detected, death_cross_detected, trend_direction, trend_strength, breakout_potential, volume_confirmation = row
                    analysis["ma_convergence_data"] = analysis.get("ma_convergence_data", {})
                    analysis["ma_convergence_data"][tf] = {
                ...
                # No Moving Average Convergence data in database - skip calculation for now
                analysis["ma_convergence_data"] = {}
        except Exception as e:
            logger.warning(f"Error reading Moving Average Convergence from database for {symbol}: {e}")
            # No fallback calculation - skip Moving Average Convergence data
            analysis["ma_convergence_data"] = {}

        # Try to get MACD Histogram data from database
        try:
            cursor.execute("""
                SELECT timeframe, histogram_trend, histogram_strength, zero_line_cross,
        ...
        # Try to get Bollinger Band Squeeze data from database
        try:
            cursor.execute("""
        ...
        # Try to get Price Action Patterns data from database
        try:
            cursor.execute("""
        ...
        # Try to get RSI Divergence data from database
        try:
            cursor.execute("""
        ...
        # Try to get Stochastic Oscillator data from database
        try:
            cursor.execute("""
        ...
        # Try to get CCI data from database
        try:
            cursor.execute("""
                SELECT timeframe, cci_value, overbought_level, oversold_level,
        ...
        # Try to get ADX data from database
        try:
            cursor.execute("""
        ...
        # Try to get Parabolic SAR data from database
        try:
            cursor.execute("""
        ...
        # Try to get ATR data from database
        try:
            cursor.execute("""
                SELECT timeframe, atr_value, atr_percentage, volatility_level,
                       volatility_strength, volatility_trend, volatility_change,
                       breakout_potential, breakout_strength, true_range,
                       high_low_range, high_close_range, low_close_range
                FROM atr_data 
                WHERE symbol = ?
                ORDER BY timeframe
            """, (symbol,))
        ...
        # Try to get Williams %R data from database
        try:
            cursor.execute("""
        ...
        # Try to get Stochastic RSI data from database
        try:
            cursor.execute("""
        ...
        # Try to get Ichimoku data from database
        try:
            cursor.execute("""
                SELECT timeframe, tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b,
                       ...
                       support_distance, resistance_distance, momentum_signal, trend_strength
                FROM ichimoku_data 
                WHERE symbol = ?
                ORDER BY timeframe
            """, (symbol,))
        ...
        # Try to get Fibonacci data from database
        try:
            cursor.execute("""
        ...
        # Try to get Volume data from database
        try:
            cursor.execute("""
            ...
            if db_volume_data:
                ...
                for row in db_volume_data:
                    ...
                    analysis["volume_data"][tf] = {
                        ...
                        "obv": obv,
                        "obv_sma": obv_sma,
                        "volume_spike_detected": bool(volume_spike_detected),
        ...
        # Try to get EMA data from database
        try:
            cursor.execute("""
                SELECT timeframe, ema_9, ema_12, ema_20, ema_21, ema_26, ema_50, 
            ...
            if db_ema_data:
                ...
                # Calculate EMA for all timeframes if not in database
                analysis["ema_data"] = {}
                for tf in timeframes:
                    tf_prices = timeframes_data.get(tf, [])
                    if len(tf_prices) >= 50:
                        ema_9 = ta_service.calculate_ema(tf_prices, 9)
                        ema_12 = ta_service.calculate_ema(tf_prices, 12)
                        ema_20 = ta_service.calculate_ema(tf_prices, 20)
                        ema_21 = ta_service.calculate_ema(tf_prices, 21)
                        ema_26 = ta_service.calculate_ema(tf_prices, 26)
                        ema_50 = ta_service.calculate_ema(tf_prices, 50)
                        ...
                        if all([ema_9, ema_12, ema_20, ema_21, ema_26, ema_50]):
                            ...
                            if ema_12[-1] > ema_26[-1] and len(ema_12) >= 2 and len(ema_26) >= 2:
                                if ema_12[-2] <= ema_26[-2]:
                                    golden_cross_detected = True
                                    cross_signal = "golden_cross"
                                    cross_strength = abs(ema_12[-1] - ema_26[-1]) / ema_26[-1]
                            elif ema_12[-1] < ema_26[-1] and len(ema_12) >= 2 and len(ema_26) >= 2:
                                if ema_12[-2] >= ema_26[-2]:
        ...
        print(f"🔍 DEBUG: EMA section completed for {symbol}")

        # Try to get RSI data from database
        print(f"🔍 DEBUG: About to fetch RSI data for {symbol}")
        try:
            cursor.execute("""
            ...
            if db_rsi_data:
                ...
                # Calculate RSI for all timeframes if not in database
                analysis["rsi_data"] = {}
                for tf in timeframes:
                    tf_prices = timeframes_data.get(tf, [])
                    if len(tf_prices) >= 15:
                        rsi_values = ta_service.calculate_rsi(tf_prices, 14)
                        if rsi_values:
        ...
        # Try to get MACD data from database
        try:
            cursor.execute("""
            ...
            if db_macd_data:
                ...
                # Calculate MACD for all timeframes if not in database
                for tf in timeframes:
                    tf_prices = timeframes_data.get(tf, [])
                    if len(tf_prices) >= 26:
                        macd_line, signal_line, histogram = ta_service.calculate_macd(tf_prices)
                        if macd_line and signal_line and histogram:
                            ...
                            if len(macd_line) >= 2 and len(signal_line) >= 2:
                                ...
                                if prev_macd <= prev_signal and macd_line[-1] > signal_line[-1]:
                                    signal_status = "Bullish Crossover"
                                elif prev_macd >= prev_signal and macd_line[-1] < signal_line[-1]:
        ...
        # Get alerts for this symbol from database
        cursor.execute("""
            SELECT alert_type, timeframe, condition, threshold, is_active, last_triggered
            FROM symbol_alerts 
        ...
        for alert in alerts:
            alert_type, timeframe, condition, threshold, is_active, last_triggered = alert
            analysis["alerts"].append({
                ...
                "distance_to_trigger": abs(current_price - threshold),
                "percentage_to_trigger": abs((current_price - threshold) / current_price * 100)
            })
